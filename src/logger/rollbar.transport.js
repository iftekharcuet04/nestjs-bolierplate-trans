  ...
  const stream = abstractTransport(
    async function (source) {
      for await (const obj of source) {
        const {context, err, msg, level, user, ...props } = obj;
        const rollbarLevel = pinoLevelToRollbar(level);

        // Skip logging GraphQLError to Rollbar
        // if (options.logErrors && rollbarLevel instanceof GraphQLError) {
        if(["ExceptionsHandler"].includes(context)){
          console.error(rollbarLevel, msg);
          continue; // Skip processing this error
        }
